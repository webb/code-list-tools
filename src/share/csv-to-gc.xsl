...
<xsl:stylesheet
   exclude-result-prefixes="c xs xsl"
   version="2.0"
  ...
  <xsl:template match="c:file/text()" priority="1"/>
  <xsl:template match="c:row/text()" priority="1"/>
  ...
  <xsl:template match="c:row[position() = 1]" priority="2"/>

  <xsl:template match="c:row" priority="1">
    <Row>
      <xsl:apply-templates/>
    </Row>
  </xsl:template>

  <xsl:template match="c:column" priority="1">
    <Value>
